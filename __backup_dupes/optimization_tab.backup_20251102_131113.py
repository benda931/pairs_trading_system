...
# =========================
# SECTION 4: Storage / DuckDB
# =========================
DUCK = duckdb.connect(str(PROJECT_ROOT / "cache.duckdb"))
...
def load_pairs(refresh: bool = False) -> List[Dict[str, Any]]:
    ...
    try:
        if pairs_path_json.exists():
            pairs = json.loads(pairs_path_json.read_text())
        elif pairs_path_yaml.exists():
            import yaml  # deferred
            pairs = yaml.safe_load(pairs_path_yaml.read_text()) or []
        else:
            pairs = [  # demo defaults
                {"symbols": ["AAPL", "MSFT"]},
                {"symbols": ["GOOG", "META"]},
...
def _render_profile_sidebar() -> Optional[OptSettings]:
    ...
    export_json = json.dumps(make_json_safe(export_dict, indent=2, ensure_ascii=False))
    try:
        import yaml  # deferred
...
def _sidebar_common() -> Dict[str, Any]:
    ...
    for k, (lo, hi, step) in ranges.items():
        lo_n = st.sidebar.number_input(f"{k} min", value=float(lo), step=float(step))
        hi_n = st.sidebar.number_input(f"{k} max", value=float(hi), step=float(step))
        edited[k] = (float(lo_n), float(hi_n), float(step))
...
def render_optimization_tab() -> None:
    ...
    essentials = {
        ...
        "optuna": optuna is not None and not isinstance(TPESampler, object),
    ...
    if st.button("Run Sanity Check (1 backtest)"):
        try:
            ...
            st.json(make_json_safe({"params": mid_params, "perf": perf}))
        except Exception as e:
            ...
            if st.button("Save Results to studies/"):
                try:
                    ...
                    if df_current.empty:
                        ...
                        out = STUDIES_DIR / f"study_{ts}.csv"
                ...
                with st.expander("AutoML Insights", expanded=False):
                    try:
                        ...
                        if isinstance(insights, pd.DataFrame):
                            ...
                            st.json(make_json_safe(insights))
                    except Exception as e:
