...
def apply_caps(weights: pd.Series, caps: Mapping[str, float], groups: Mapping[str, str]) -> pd.Series:
    ...
    for g, cap in caps.items():
        ...
        if over > 1e-12:
            ...
            take_sum = take.sum()
            if take_sum > 0:
                reduce = (take / take_sum) * over
            else:
                reduce = 0.0
            df.loc[mask, "w"] = (take - reduce).clip(lower=0.0)
            # redistribute to non-mask
            rest_mask = ~mask
            rest_sum = df.loc[rest_mask, "w"].sum() if rest_mask.any() else 0.0
            if rest_mask.any() and rest_sum > 0:
                df.loc[rest_mask, "w"] += df.loc[rest_mask, "w"] / rest_sum * over
...
class DrawdownGate:
    ...
    def update(self, equity: float) -> Tuple[bool, float]:
        ...
        if np.isnan(equity):
            return False, 0.0
        equity = float(equity)
        if equity <= 0 or self._peak <= 0:
            # Negative or zero equity: treat as 100% drawdown
            return True, 1.0
