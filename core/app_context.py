...
class AppContext:
    ...
    def init_services(self) -> None:
        """
        Initialize service layer (SqlStore, MarketDataRouter, Engines).

        Delegates to core.context_services.init_all_services() which handles
        each service independently with graceful degradation.

        Extension points:
            - Override SqlStore/MarketDataRouter via set_service() before calling.
            - Replace Broker/IBRouter via self.broker/self.ib_router.
        """
        from core.context_services import init_all_services
        init_all_services(self)

    def init_ib_router(self) -> None:
        """Initialize IBKR order router (delegates to context_services)."""
        from core.context_services import _init_ib_router
        _init_ib_router(self)
