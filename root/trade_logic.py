...
def apply_transaction_costs(
    returns,        # pd.Series
    position,       # pd.Series (signal/position: -1/0/1)
    price_changes,  # pd.Series (underlying returns proxy)
    params: Dict[str, Any],
):
    """
    Apply simple transaction cost model to a return series.

    This is a backward-compat helper expected by legacy optimizers
    (e.g. core/full_parameter_optimization.py).

    Inputs
    ------
    returns:
        Per-period strategy returns (already position * price_changes).
    position:
        Target position series (e.g., -1/0/1). Costs are applied on position changes.
    price_changes:
        Underlying return proxy (not strictly required, but kept for signature compatibility).
    params:
        Dict with optional keys:
            - slippage_bps (float): bps cost per unit turnover
            - transaction_cost_bps (float): bps cost per unit turnover
            - transaction_cost_per_trade (float): commission dollars per trade (converted using notional)
            - notional (float): dollar notional used for conversion (default 100000)

    Returns
    -------
    pd.Series
        returns_net = returns - cost, aligned to index.
    """
    # Local import to keep root.trade_logic lightweight for non-pandas usage paths
    import pandas as pd
    import numpy as np
    ...
    p = pd.Series(position).reindex(r.index).fillna(0.0)

    # Turnover proxy: abs(change in position)
    # - 0 -> 1 : 1 trade unit
    # - 1 -> -1: 2 trade units (close + open)
    turnover = p.diff().abs()
    if len(turnover) > 0:
        turnover.iloc[0] = abs(p.iloc[0])  # opening cost at t0 if position starts non-zero
    turnover = turnover.fillna(0.0)

    slippage_bps = float(params.get("slippage_bps", params.get("slippage", 0.0)) or 0.0)
    tc_bps = float(params.get("transaction_cost_bps", params.get("tc_bps", 0.0)) or 0.0)

    # Dollar commission per trade -> convert to return units using notional
    tc_per_trade = float(params.get("transaction_cost_per_trade", 0.0) or 0.0)
    notional = float(params.get("notional", 100000.0) or 100000.0)
    if not np.isfinite(notional) or notional <= 0:
        notional = 100000.0

    cost_per_turnover = 0.0
    if slippage_bps:
        cost_per_turnover += slippage_bps / 10000.0
    if tc_bps:
        cost_per_turnover += tc_bps / 10000.0
    if tc_per_trade:
        cost_per_turnover += (tc_per_trade / notional)

    if cost_per_turnover <= 0:
        return r

    costs = turnover * cost_per_turnover
    r_net = (r - costs).replace([np.inf, -np.inf], np.nan).fillna(0.0)
    return r_net
