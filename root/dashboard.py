...
# ─── Shared tab rendering helper ──────────────────────────────────
# Eliminates boilerplate: log + prewarm + delegate + telemetry

def _render_standard_tab(
    tab_key: TabKey,
    logical_name: str,
    module_candidates: tuple,
    func_candidates: tuple,
    app_ctx: "AppContext",
    feature_flags: FeatureFlags,
    nav_payload: Optional[NavPayload] = None,
    *,
    enrich_nav: Optional[Dict[str, Any]] = None,
) -> None:
    """
    Standard tab render pattern: log entry, enrich nav, delegate, handle errors.

    This helper reduces each render_* wrapper from 100-400 lines to ~15 lines
    by encapsulating the common boilerplate (prewarm, nav enrichment, telemetry).
    """
    import time as _t
    _log_tab_entry(tab_key, feature_flags, nav_payload)
    t0 = _t.time()

    # Merge any enrichment into nav_payload
    effective_nav: Dict[str, Any] = {}
    if isinstance(nav_payload, dict):
        effective_nav.update(nav_payload)
    effective_nav.setdefault("source_tab", logical_name)
    if enrich_nav:
        for k, v in enrich_nav.items():
            effective_nav.setdefault(k, v)
    ...
    try:
        _lazy_render_tab(
            ...
            app_ctx=app_ctx,
            feature_flags=feature_flags,
            nav_payload=effective_nav,
        )
    except Exception as exc:
        logger.exception("%s tab rendering failed: %s", logical_name, exc)
        try:
            st.error(f"Tab '{logical_name}' failed to render: {exc}")
        except Exception:
            pass
    ...
    try:
        elapsed = _t.time() - t0
        st.session_state[f"{logical_name}_tab_last_runtime_sec"] = float(elapsed)
        logger.info("%s tab rendered in %.3f sec", logical_name, elapsed)
    except Exception:
        pass
...
def render_smart_scan_tab(
    ...
    """Delegates to external smart_scan tab module."""
    _render_standard_tab(
        tab_key=TAB_KEY_SMART_SCAN,
        logical_name="smart_scan",
        module_candidates=("smart_scan_tab", "root.smart_scan_tab"),
        func_candidates=("render_smart_scan_tab", "render_tab"),
        app_ctx=app_ctx,
        feature_flags=feature_flags,
        nav_payload=nav_payload,
    )
...
def render_pair_tab(
    ...
    """Delegates to external pair tab module."""
    _render_standard_tab(
        tab_key=TAB_KEY_PAIR,
        logical_name="pair",
        module_candidates=("pair_tab", "root.pair_tab"),
        func_candidates=("render_pair_tab", "render_tab"),
        app_ctx=app_ctx,
        feature_flags=feature_flags,
        nav_payload=nav_payload,
    )
...
def render_matrix_tab(
    ...
    """Delegates to external matrix tab module."""
    _render_standard_tab(
        tab_key=TAB_KEY_MATRIX,
        logical_name="matrix",
        module_candidates=("matrix_research_tab", "root.matrix_research_tab"),
        func_candidates=("render_matrix_tab", "render_tab"),
        app_ctx=app_ctx,
        feature_flags=feature_flags,
        nav_payload=nav_payload,
    )
...
def render_comparison_matrices_tab(
    ...
    """Delegates to external comparison_matrices tab module."""
    _render_standard_tab(
        tab_key=TAB_KEY_COMPARISON_MATRICES,
        logical_name="comparison_matrices",
        module_candidates=("tab_comparison_matrices", "root.tab_comparison_matrices"),
        func_candidates=("render_comparison_matrices_tab", "render_tab"),
        app_ctx=app_ctx,
        feature_flags=feature_flags,
        nav_payload=nav_payload,
    )
...
def render_backtest_tab(
    ...
    """Delegates to external backtest tab module."""
    _render_standard_tab(
        tab_key=TAB_KEY_BACKTEST,
        logical_name="backtest",
        module_candidates=("backtest_tab", "root.backtest_tab", "backtest", "root.backtest"),
        func_candidates=("render_backtest_tab", "render_tab"),
        app_ctx=app_ctx,
        feature_flags=feature_flags,
        nav_payload=nav_payload,
    )
...
def render_optimization_tab(
    ...
    """Delegates to external optimization tab module."""
    _render_standard_tab(
        tab_key=TAB_KEY_OPTIMIZATION,
        logical_name="optimization",
        module_candidates=("optimization_tab", "root.optimization_tab"),
        func_candidates=("render_optimization_tab", "render_tab"),
        app_ctx=app_ctx,
        feature_flags=feature_flags,
        nav_payload=nav_payload,
    )
...
def render_insights_tab(
    ...
    """Delegates to external insights tab module."""
    _render_standard_tab(
        tab_key=TAB_KEY_INSIGHTS,
        logical_name="insights",
        module_candidates=("insights_tab", "root.insights_tab", "insights", "root.insights"),
        func_candidates=("render_insights_tab", "render_tab"),
        app_ctx=app_ctx,
        feature_flags=feature_flags,
        nav_payload=nav_payload,
    )
...
def render_macro_tab(
    ...
    """Delegates to external macro_tab module."""
    _render_standard_tab(
        tab_key=TAB_KEY_MACRO,
        logical_name="macro",
        module_candidates=("macro_tab", "root.macro_tab", "macro", "root.macro"),
        func_candidates=("render_macro_tab", "render_tab"),
        app_ctx=app_ctx,
        feature_flags=feature_flags,
        nav_payload=nav_payload,
    )
...
def render_portfolio_tab(
    ...
    """Delegates to external portfolio_tab module."""
    _render_standard_tab(
        tab_key=TAB_KEY_PORTFOLIO,
        logical_name="portfolio",
        module_candidates=("portfolio_tab", "root.portfolio_tab"),
        func_candidates=("render_portfolio_tab", "render_tab"),
        app_ctx=app_ctx,
        feature_flags=feature_flags,
        nav_payload=nav_payload,
    )
...
def render_risk_tab(
    ...
    """Delegates to external risk_tab module."""
    _render_standard_tab(
        tab_key=TAB_KEY_RISK,
        logical_name="risk",
        module_candidates=("risk_tab", "root.risk_tab"),
        func_candidates=("render_risk_tab", "render_tab"),
        app_ctx=app_ctx,
        feature_flags=feature_flags,
        nav_payload=nav_payload,
    )
...
def render_fair_value_tab(
    ...
    """💎 Fair Value tab — delegates to fair_value_api_tab module."""
    _render_standard_tab(
        tab_key=TAB_KEY_FAIR_VALUE,
        logical_name="fair_value",
        module_candidates=("fair_value_api_tab", "root.fair_value_api_tab"),
        func_candidates=("render_fair_value_tab", "render_tab"),
        app_ctx=app_ctx,
        feature_flags=feature_flags,
        nav_payload=nav_payload,
    )
...
# =====================
# Parts 9-10/35 – Service status + header (extracted to root/dashboard_ui_components.py)
# =====================
from root.dashboard_ui_components import *  # noqa: F401,F403
# Part 11/35 – Cache & quick actions (extracted to root/dashboard_cache.py)
# =====================
from root.dashboard_cache import *  # noqa: F401,F403
# Part 12/35 – Sidebar (extracted to root/dashboard_sidebar.py)
# =====================
from root.dashboard_sidebar import *  # noqa: F401,F403
...
# =====================
# Part 17/35 – URL query params bridge (extracted to root/dashboard_url_router.py)
# =====================
from root.dashboard_url_router import *  # noqa: F401,F403
...
# =====================
# Part 19/35 – Agent/Consumer context (extracted to root/dashboard_agent_context.py)
# =====================
from root.dashboard_agent_context import *  # noqa: F401,F403
# Part 20/35 – DashboardSummary & Telemetry (extracted to root/dashboard_telemetry.py)
# =====================
from root.dashboard_telemetry import (  # noqa: F401
...
try:
    __all__ += [
        ...
        "dashboard_summary_to_dict", "update_dashboard_summary_in_session",
        "export_dashboard_summary",
    ]
except NameError:
    pass
# Part 21/35 – User preferences (extracted to root/dashboard_preferences.py)
# =====================
from root.dashboard_preferences import *  # noqa: F401,F403
# Part 22/35 – Dashboard toolbar (extracted to root/dashboard_toolbar.py)
# =====================
from root.dashboard_toolbar import *  # noqa: F401,F403
# Part 23/35 – Logs Tab (extracted to root/dashboard_logs_tab.py)
# =====================
from root.dashboard_logs_tab import (  # noqa: F401
    render_logs_tab,
)
# Part 24/35 – Health API (extracted to root/dashboard_health.py)
# =====================
from root.dashboard_health import (  # noqa: F401
...
try:
    __all__ += [
        ...
        "dashboard_health_to_dict", "check_dashboard_ready",
    ]
except NameError:
    pass
# Part 25/35 – Unified Dashboard API bundle (extracted to root/dashboard_api_bundle.py)
# =====================
from root.dashboard_api_bundle import *  # noqa: F401,F403
# Part 26/35 – Developer diagnostics (extracted to root/dashboard_diagnostics.py)
# =====================
from root.dashboard_diagnostics import *  # noqa: F401,F403
# Parts 27-29/35 – Desktop Bridge + Agent Router + Saved Views
# (extracted to root/dashboard_integrations.py)
# =====================
from root.dashboard_integrations import (  # noqa: F401
    # Part 27 — Desktop bridge
...
try:
    __all__ += [
        "DashboardDesktopBridgeConfig", "build_desktop_integration_config",
        "build_desktop_payload_from_runtime", "push_dashboard_state_to_desktop_if_available",
        "get_last_desktop_push_info", "AgentAction", "handle_agent_action",
        "handle_agent_actions_batch", "get_agent_actions_history_tail",
        "SavedDashboardView", "list_saved_views", "add_saved_view_from_runtime",
        "find_saved_view_by_name", "apply_saved_view", "export_saved_views_for_agents",
        "persist_saved_views_to_sql_store",
    ]
except NameError:
    pass
# Part 30/35 – Agents Tab (extracted to root/dashboard_agents_tab.py)
# =====================
from root.dashboard_agents_tab import (  # noqa: F401
    render_agents_tab,
)
# Part 31/35 – Headless/Testing helpers (extracted to root/dashboard_headless.py)
# =====================
from root.dashboard_headless import *  # noqa: F401,F403
...
# Part 33/35 – Overview metrics (extracted to root/dashboard_metrics.py)
# =====================
from root.dashboard_metrics import (  # noqa: F401
...
try:
    __all__ += [
        "OverviewMetric", "build_dashboard_overview_metrics",
        "dashboard_overview_metrics_to_dict",
    ]
except NameError:
    pass
# Part 34/35 – Home data helpers (extracted to root/dashboard_home_helpers.py)
# =====================
from root.dashboard_home_helpers import *  # noqa: F401,F403
